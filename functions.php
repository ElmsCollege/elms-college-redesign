...
function gs_elms_scripts() {
	...
	wp_enqueue_style( 'jquery-ui', get_template_directory_uri() . '/css/jquery-ui.min.css', array(), '20200219');
  ...
  function left_rail_nav_collapse( $post_id, $current_level, $parentId ) {
    ...
    foreach ( $children as $child ) {
      ...
      if ( count( $next_generation ) > 0 ) {
        ...
        echo '<li>';
        echo '<a href="' . get_permalink( $child->ID ) . '" class="' . $current_level . '-child"' . ( ( $curId == $child->ID ) ? ' current_page_item' : '' ) . '">' . get_the_title( $child->ID ) . '</a>';
      };
